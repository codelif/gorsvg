...
render_result_t *render_svg_css(const char *svg_data, int svg_len,
                                const char *css, int width, int height) {
  GError *error = NULL;
  RsvgHandle *handle =
      rsvg_handle_new_from_data((const guint8 *)svg_data, svg_len, &error);
  if (error || !handle) {
    if (error)
      g_error_free(error);
    return NULL;
  ...
  if (!rsvg_handle_get_intrinsic_size_in_pixels(handle, &svg_width,
                                                &svg_height)) {
    // fallback
    viewport.x = 0;
    viewport.y = 0;
    ...
    if (rsvg_handle_get_geometry_for_layer(handle, NULL, &viewport, &ink_rect,
                                           &logical_rect, &error)) {
...
int set_svg_stylesheet(RsvgHandle *handle, const char *css) {
  GError *error = NULL;
  gboolean success = rsvg_handle_set_stylesheet(handle, (const guint8 *)css,
                                                strlen(css), &error);
  if (error) {
    g_error_free(error);
    return 0;
